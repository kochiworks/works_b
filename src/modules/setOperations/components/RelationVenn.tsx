 ...
 *
 * Each opening is angled so the two names never land near each other: the
 * inner curve opens at the top, the outer one to the upper left.
 */
const LAYOUTS: Record<RelationVerdict['layout'], Layout> = {
  overlap: {
    ...
    centroids: { A: { x: 103, y: 130 }, AB: { x: 180, y: 130 }, B: { x: 257, y: 130 }, '': OUTSIDE },
  },
  disjoint: {
    circles: { A: { cx: 103, cy: 130, r: 66 }, B: { cx: 257, cy: 130, r: 66 } },
  ...
  equal: {
    ...
    centroids: { A: { x: 180, y: 134 }, AB: { x: 180, y: 134 }, B: { x: 180, y: 134 }, '': OUTSIDE },
    merged: true,
  },
}

/** The universe box, opened on its top edge so U can sit in the opening. */
function framePath(gapWidth: number): string {
  const { x, y, w, h, radius } = FRAME
  const right = x + w
  const bottom = y + h
  const gapFrom = UNIVERSE_LABEL.x - gapWidth / 2
  const gapTo = UNIVERSE_LABEL.x + gapWidth / 2
  return [
    `M ${gapTo} ${y}`,
    `H ${right - radius}`,
...
export function RelationVenn({ verdict, universe, membership }: Props) {
  ...
  const gapWidth = layout.merged ? WIDE_NAME_GAP : NAME_GAP
  ...
  return (
    <figure className="venn">
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${verdict.title} 벤 다이어그램`}>
        <path className="venn-frame" d={framePath(NAME_GAP)} />
          ...
          return (
            <Fragment key={name}>
              <path
                ...
                d={openArcPath(circle, angle, gapWidth)}
                data-set={name}
                data-cx={circle.cx}
                data-cy={circle.cy}
                data-r={circle.r}
